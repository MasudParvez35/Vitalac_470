...
public partial class WebWorkContext : IWorkContext
{
    ...
    protected Merchant _cachedMerchant;
    ...
    /// <summary>
    /// Gets the current merchant (logged-in manager)
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task<Merchant> GetCurrentMerchantAsync()
    {
        //whether there is a cached value
        ...
        var customer = await GetCurrentCustomerAsync();
        if (customer == null || customer.MerchantId < 1)
            return null;

        //check merchant availability
        var merchant = await _merchantService.GetMerchantByIdAsync(customer.MerchantId);
        if (merchant == null || merchant.Deleted || !merchant.Active)
            return null;

        //cache the found merchant
        _cachedMerchant = merchant;

        return _cachedMerchant;
    }

    /// <summary>
    /// Gets the location of current customer
    /// </summary>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task<Location> GetCurrentLocationAsync()
    {
        //whether there is a cached value
        ...
        var merchant = await _merchantService.GetMerchantByAreaIdAsync(customer.AreaId);

        return _cachedLocation = new()
        {
