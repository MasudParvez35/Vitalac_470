...
public partial class OrderModelFactory : IOrderModelFactory
{
    ...
    protected virtual async Task PrepareOrderModelPaymentInfoAsync(OrderModel model, Order order)
    {
        ...
        model.BillingAddress.ThanaName = (await _thanaService.GetThanaByAddressAsync(billingAddress))?.Name;
        model.BillingAddress.AreaName = (await _areaService.GetAreaByAddressAsync(billingAddress))?.Name;
    ...
    protected virtual async Task PrepareOrderModelShippingInfoAsync(OrderModel model, Order order)
    {
        ...
        if (!order.PickupInStore)
        {
            ...
            model.ShippingAddress.ThanaName = (await _thanaService.GetThanaByAddressAsync(shippingAddress))?.Name;
            model.ShippingAddress.AreaName = (await _areaService.GetAreaByAddressAsync(shippingAddress))?.Name;
    ...
    public virtual async Task<OrderSearchModel> PrepareOrderSearchModelAsync(OrderSearchModel searchModel)
    {
        ...
        await _baseAdminModelFactory.PrepareMerchantsAsync(searchModel.AvailableMerchants);
    ...
    public virtual async Task<OrderListModel> PrepareOrderListModelAsync(OrderSearchModel searchModel)
    {
        ...
        var currentMerchant = await _workContext.GetCurrentMerchantAsync();
        if (currentMerchant != null)
            searchModel.MerchantId = currentMerchant.Id;
    ...
    public virtual async Task<OrderModel> PrepareOrderModelAsync(OrderModel model, Order order, bool excludeProperties = false)
    {
        if (order != null)
        {
            ...
            model.MerchantId = order.MerchantId;
            if (model.MerchantId > 0)
            {
                var merchant = await _merchantService.GetMerchantByIdAsync(model.MerchantId);
                model.MerchantName = merchant?.Name;
            }
