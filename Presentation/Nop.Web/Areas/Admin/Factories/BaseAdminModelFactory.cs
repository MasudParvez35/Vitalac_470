...
public partial class BaseAdminModelFactory : IBaseAdminModelFactory
{
    ...
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the merchant list
    /// </returns>
    protected virtual async Task<List<SelectListItem>> GetMerchantListAsync()
    {
        var merchants = await _staticCacheManager.GetAsync(NopModelCacheDefaults.MerchantsListKey, async () =>
        {
            return await _merchantService.GetAllMerchantsAsync(showHidden: true);
        });
        return merchants.ToSelectListItems(x => x.Name);
    }
    ...
    /// Prepare thanas
    /// </summary>
    /// <param name="items">thana items</param>
    /// <param name="StateProvinceId">state identifier; pass null to don't load thana</param>
    /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
    /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task PrepareThanasAsync(IList<SelectListItem> items, int? stateProvinceId, 
        bool withSpecialDefaultItem = true, string defaultItemText = null)
    {
        ArgumentNullException.ThrowIfNull(items);
        if (stateProvinceId.HasValue)
        {
            //prepare available thanas of the stateprovince
            var availableThanas = await _thanaService.GetThanasByStateProvinceIdAsync(stateProvinceId.Value, showHidden: true);
            foreach (var thana in availableThanas)
            {
                items.Add(new SelectListItem { Value = thana.Id.ToString(), Text = thana.Name });
            }

            //insert special item for the default value
            if (items.Count > 1)
                await PrepareDefaultItemAsync(items, withSpecialDefaultItem, defaultItemText ?? await _localizationService.GetResourceAsync("Admin.Address.SelectThana"));
    ...
    /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task PrepareAreasAsync(IList<SelectListItem> items, int? thanaId, 
        bool withSpecialDefaultItem = true, string defaultItemText = null)
    {
        ArgumentNullException.ThrowIfNull(items);
        if (thanaId.HasValue)
        {
            // prepare available areas of the thana
            var availableAreas = await _areaService.GetAreasByThanaIdAsync(thanaId.Value, showHidden: true);
            foreach (var area in availableAreas)
            {
                items.Add(new SelectListItem { Value = area.Id.ToString(), Text = area.Name });
            }
            // insert special item for the default value
            if (items.Count > 1)
                await PrepareDefaultItemAsync(items, withSpecialDefaultItem, defaultItemText ?? await _localizationService.GetResourceAsync("Admin.Address.SelectArea"));
    ...
    /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
    /// <returns>A task that represents the asynchronous operation</returns>
    public virtual async Task PrepareMerchantsAsync(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
    {
        ArgumentNullException.ThrowIfNull(items);

        //prepare available merchants
        var availableMerchantItems = await GetMerchantListAsync();
        foreach (var merchantItem in availableMerchantItems)
        {
