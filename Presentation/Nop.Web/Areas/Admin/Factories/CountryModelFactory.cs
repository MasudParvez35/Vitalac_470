...
public partial class CountryModelFactory : ICountryModelFactory
{
    ...
    /// <summary>
    /// Prepare thana search model
    /// </summary>
    /// <param name="searchModel">State and province search model</param>
    /// <param name="stateProvince">stateProvince</param>
    /// <returns>thana search model</returns>
    protected virtual ThanaSearchModel PrepareThanaSearchModel(ThanaSearchModel searchModel, StateProvince stateProvince)
    {
        ArgumentNullException.ThrowIfNull(searchModel);
    ...
    /// <summary>
    /// Prepare area search model
    /// </summary>
    /// <param name="searchModel">thana search model</param>
    /// <param name="thana">thana</param>
    /// <returns>area search model</returns>
    protected virtual AreaSearchModel PrepareAreaSearchModel(AreaSearchModel searchModel, Thana thana)
    {
        ArgumentNullException.ThrowIfNull(searchModel);

        ArgumentNullException.ThrowIfNull(thana);

        searchModel.ThanaId = thana.Id;
    ...
    public virtual async Task<StateProvinceListModel> PrepareStateProvinceListModelAsync(StateProvinceSearchModel searchModel, Country country)
    {
        ...
        // prepare list model
        var model = await new StateProvinceListModel().PrepareToGridAsync(searchModel, states, () =>
        {
            // fill in model values from the entity
            return states.SelectAwait(async state =>
            {
                var stateModel = state.ToModel<StateProvinceModel>();

                stateModel.NumberOfThanas = (await _thanaService.GetThanasByStateProvinceIdAsync(state.Id))?.Count ?? 0;
        ...
        Country country, StateProvince state, bool excludeProperties = false)
    {
        ...
        if (state != null)
        {
            // fill in fill in model values from the entity
            if (model == null)
            {
                model = state.ToModel<StateProvinceModel>();
                model.NumberOfThanas = (await _thanaService.GetThanasByStateProvinceIdAsync(state.Id))?.Count ?? 0;
            }

            //prepare nested search model
            PrepareThanaSearchModel(model.ThanaSearchModel, state);
    ...
    public virtual async Task<ThanaListModel> PrepareThanaListModelAsync(ThanaSearchModel searchModel, StateProvince stateProvince)
    {
        ArgumentNullException.ThrowIfNull(searchModel);
        //ArgumentNullException.ThrowIfNull(stateProvince);

        // get thanas
        var thanas = (await _thanaService.GetThanasByStateProvinceIdAsync(stateProvince.Id, showHidden: true)).ToPagedList(searchModel);

        // prepare list model
        var model = await new ThanaListModel().PrepareToGridAsync(searchModel, thanas, () =>
        {
            // fill in model values from the entity
            return thanas.SelectAwait(async thana =>
            {
                var thanaModel = thana.ToModel<ThanaModel>();

                thanaModel.NumberOfAreas = (await _areaService.GetAreasByThanaIdAsync(thana.Id))?.Count ?? 0;
    ...
    public virtual async Task<ThanaModel> PrepareThanaModelAsync(ThanaModel model, StateProvince stateProvince, 
        Thana thana, bool excludeProperties = false)
    {
        Func<ThanaLocalizedModel, int, Task> localizedModelConfiguration = null;
        ...
        if (thana != null)
        {
            ...
            if (model == null)
            {
                model = thana.ToModel<ThanaModel>();
                model.NumberOfAreas = (await _areaService.GetAreasByThanaIdAsync(thana.Id))?.Count ?? 0;
            }

            //prepare nested search model
            PrepareAreaSearchModel(model.AreaSearchModel, thana);

            //define localized model configuration action
            localizedModelConfiguration = async (locale, languageId) =>
            {
                locale.Name = await _localizationService.GetLocalizedAsync(thana, entity => entity.Name, languageId, false, false);
            };
        }
    ...
    public virtual async Task<AreaListModel> PrepareAreaListModelAsync(AreaSearchModel searchModel, Thana thana)
    {
        ArgumentNullException.ThrowIfNull(searchModel);
        ArgumentNullException.ThrowIfNull(thana);

        //get comments
        var areas = (await _areaService.GetAreasByThanaIdAsync(thana.Id, showHidden: true)).ToPagedList(searchModel);

        //prepare list model
        var model = new AreaListModel().PrepareToGrid(searchModel, areas, () =>
        {
            //fill in model values from the entity
            return areas.Select(area => area.ToModel<AreaModel>());
        });

        return model;
    }

    public virtual async Task<AreaModel> PrepareAreaModelAsync(AreaModel model, Thana thana, Area area, bool excludeProperties = false)
    {
        Func<AreaLocalizedModel, int, Task> localizedModelConfiguration = null;

        if (area != null)
        {
            //fill in model values from the entity
            model ??= area.ToModel<AreaModel>();

            //define localized model configuration action
            localizedModelConfiguration = async (locale, languageId) =>
            {
                locale.Name = await _localizationService.GetLocalizedAsync(area, entity => entity.Name, languageId, false, false);
            };
        }

        model.ThanaId = thana.Id;
