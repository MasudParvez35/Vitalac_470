...
public partial class AddressModelFactory : IAddressModelFactory
{
        ...
        string overrideAttributesXml = "")
    {
        ...
        if (!excludeProperties && address != null)
        {
            ...
            model.ThanaId = address.ThanaId;
            model.ThanaName = await _thanaService.GetThanaByAddressAsync(address) is Thana thana ? await _localizationService.GetLocalizedAsync(thana, x => x.Name) : null;
            model.AreaId = address.AreaId;
            model.AreaName = await _areaService.GetAreaByAddressAsync(address) is Area area ? await _localizationService.GetLocalizedAsync(area, x => x.Name) : null;
        ...
        if (addressSettings.CountryEnabled && loadCountries != null)
        {
            ...
            if (addressSettings.DefaultCountryId != null)
                model.CountryId ??= addressSettings.DefaultCountryId;
            ...
            if (addressSettings.ThanaEnabled)
            {
                var thanas = (await _thanaService.GetThanasByStateProvinceIdAsync(model.StateProvinceId ?? 0, languageId))
                    .ToList();  
                if (thanas.Any())
                {
                    model.AvailableThanas.Add(new SelectListItem { Text = await _localizationService.GetResourceAsync("Address.SelectThana"), Value = "0" });

                    foreach (var t in thanas)
                    {
                        model.AvailableThanas.Add(new SelectListItem
                        {
                            Text = await _localizationService.GetLocalizedAsync(t, x => x.Name),
                            Value = t.Id.ToString(),
                            Selected = (t.Id == model.ThanaId)
                        });
                    }
                }
                else
                {
                    ...
                    model.AvailableThanas.Add(new SelectListItem
                    {
                        Text = await _localizationService.GetResourceAsync(anyStateSelected ? "Address.Other" : "Address.SelectThana"),
                        Value = "0"
                    });
                }
            }

            if (addressSettings.AreaEnabled)
            {
                var areas = (await _areaService.GetAreasByThanaIdAsync(model.ThanaId ?? 0, languageId))
                    .ToList();
                if (areas.Any())
                {
                    model.AvailableAreas.Add(new SelectListItem { Text = await _localizationService.GetResourceAsync("Address.SelectArea"), Value = "0" });

                    foreach (var a in areas)
                    {
                        model.AvailableAreas.Add(new SelectListItem
                        {
                            Text = await _localizationService.GetLocalizedAsync(a, x => x.Name),
                            Value = a.Id.ToString(),
                ...
                {
                    var anyThanaSelected = model.AvailableThanas.Any(x => x.Selected);
                    model.AvailableAreas.Add(new SelectListItem
                    {
                        Text = await _localizationService.GetResourceAsync(anyThanaSelected ? "Address.Other" : "Address.SelectArea"),
                        Value = "0"
                    });
                }
            }
        ...
        model.ThanaEnabled = addressSettings.ThanaEnabled;
        model.AreaEnabled = addressSettings.AreaEnabled;
