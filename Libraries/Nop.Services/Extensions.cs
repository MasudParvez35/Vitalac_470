...
public static class Extensions
{
    ...
    /// <param name="selector">Selector for name</param>
    /// <returns>SelectList</returns>
    public static List<SelectListItem> ToSelectListItems<T>(this IEnumerable<T> objList, Func<T, string> selector) where T : BaseEntity
    {
        return objList.Select(x => new SelectListItem
        {
            Text = selector(x),
            Value = x.Id.ToString()
        }).ToList();
