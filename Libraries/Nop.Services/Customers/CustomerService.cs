...
public partial class CustomerService : ICustomerService
{
    ...
    public virtual async Task<IPagedList<Customer>> GetAllCustomersAsync(DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
        ...
        int affiliateId = 0, int merchantId = 0, int vendorId = 0,
        int[] customerRoleIds = null, string email = null, string username = null, string firstName = null,
        string lastName = null, int dayOfBirth = 0,
        ...
        string ipAddress = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false)
    {
        var customers = await _customerRepository.GetAllPagedAsync(query =>
        {
            ...
            if (merchantId > 0)
                query = query.Where(c => merchantId == c.MerchantId);
    ...
    /// <summary>
    /// Gets a value indicating whether customer is merchant
    /// </summary>
    /// <param name="customer">Customer</param>
    /// <param name="onlyActiveCustomerRoles">A value indicating whether we should look only in active customer roles</param>
    /// <returns>
    ...
    /// The task result contains the result
    /// </returns>
    public virtual async Task<bool> IsMerchantAsync(Customer customer, bool onlyActiveCustomerRoles = true)
    {
        return await IsInCustomerRoleAsync(customer, NopCustomerDefaults.MerchantsRoleName, onlyActiveCustomerRoles);
    }
