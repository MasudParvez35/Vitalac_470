...
public partial class AddressService : IAddressService
{
    ...
    /// <summary>
    /// Gets total number of addresses by thana identifier
    /// </summary>
    /// <param name="thanaId">thana identifier</param>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the number of addresses
    /// </returns>
    public virtual async Task<int> GetAddressTotalByThanaIdAsync(int thanaId)
    {
        if (thanaId == 0)
            return 0;

        var query = from a in _addressRepository.Table
            where a.ThanaId == thanaId
            select a;
    ...
    /// <summary>
    /// Gets total number of addresses by area identifier
    /// </summary>
    ...
    /// The task result contains the number of addresses
    /// </returns>
    public virtual async Task<int> GetAddressTotalByAreaIdAsync(int areaId)
    {
        if (areaId == 0)
            return 0;
    ...
    public virtual async Task<bool> IsAddressValidAsync(Address address)
    {
        ...
        if (_addressSettings.CountryEnabled)
        {
            ...
            if (_addressSettings.StateProvinceEnabled)
            {
                ...
                if (_addressSettings.ThanaEnabled)
                {
                    var thanas = await _thanaService.GetThanasByStateProvinceIdAsync(address.StateProvinceId ?? 0);
                    if (thanas.Any())
                    {
                        if (address.ThanaId == null || address.ThanaId.Value == 0)
                            return false;

                        var thana = thanas.FirstOrDefault(x => address.ThanaId.HasValue && x.Id == address.ThanaId.Value);
                        if (thana == null)
                            return false;
                    }

                    if (_addressSettings.AreaEnabled)
                    {
                        var areas = await _areaService.GetAreasByThanaIdAsync(address.ThanaId ?? 0);
                        if (areas.Any())
                        {
                            if (address.AreaId == null || address.AreaId.Value == 0)
                                return false;

                            var area = areas.FirstOrDefault(x => address.AreaId.HasValue && x.Id == address.AreaId.Value);
                            if (area == null)
                                return false;
                        }
                    }
    ...
    public virtual Address FindAddress(List<Address> source, string firstName, string lastName, string phoneNumber, string email,
        ...
        string zipPostalCode, int? countryId, int? thanaId, int? areaId, string customAttributes)
    {
        return source.Find(a => ((string.IsNullOrEmpty(a.FirstName) && string.IsNullOrEmpty(firstName)) || a.FirstName == firstName) &&
                                ...
                                ((a.ThanaId == null && thanaId == null) || (a.ThanaId != null && a.ThanaId == thanaId)) &&
                                ((a.AreaId == null && areaId == null) || (a.AreaId != null && a.AreaId == areaId)) &&
    ...
    public virtual async Task<(string, KeyValuePair<AddressField, string>[])> FormatAddressAsync(Address address, int languageId = 0, string separator = ", ", bool htmlEncode = false)
    {
        ...
        var thana = await _thanaService.GetThanaByAddressAsync(address);
        var thanaName = thana != null ? await _localizationService.GetLocalizedAsync(thana, x => x.Name, languageId) : string.Empty;
        var area = await _areaService.GetAreaByAddressAsync(address);
        var areaName = area != null ? await _localizationService.GetLocalizedAsync(area, x => x.Name, languageId) : string.Empty;
        ...
        foreach (var item in indexArray)
        {
            switch ((AddressField)item)
            {
                ...
                case AddressField.Thana:
                    if (_addressSettings.ThanaEnabled && !string.IsNullOrWhiteSpace(thanaName))
                        fieldsList[indexItem] = new KeyValuePair<AddressField, string>(AddressField.Thana, htmlEncode ? WebUtility.HtmlEncode(thanaName) : thanaName);
                    else
                        fieldsList[indexItem] = new KeyValuePair<AddressField, string>(AddressField.Thana, string.Empty);
                    break;

                case AddressField.Area:
                    if (_addressSettings.AreaEnabled && !string.IsNullOrWhiteSpace(areaName))
                        fieldsList[indexItem] = new KeyValuePair<AddressField, string>(AddressField.Area, htmlEncode ? WebUtility.HtmlEncode(areaName) : areaName);
                    else
                        fieldsList[indexItem] = new KeyValuePair<AddressField, string>(AddressField.Area, string.Empty);
                    break;
