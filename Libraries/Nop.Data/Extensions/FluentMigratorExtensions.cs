...
public static class FluentMigratorExtensions
{
    ...
    /// </summary>
    /// <param name="expressionRoot">The root expression for a SCHEMA operation</param>
    /// <typeparam name="TEntity">Entity type</typeparam>
    public static ISchemaTableSyntax TableFor<TEntity>(this ISchemaExpressionRoot expressionRoot) where TEntity : BaseEntity
    {
        var type = typeof(TEntity);
        return expressionRoot.Table(NameCompatibilityManager.GetTableName(type));
    }
    ...
    /// </summary>
    /// <param name="expressionRoot">The root expression for a SCHEMA operation</param>
    /// <param name="columnExpression">Selector for column</param>
    /// <typeparam name="TEntity">Entity type</typeparam>
    public static ISchemaColumnSyntax ColumnFor<TEntity>(this ISchemaExpressionRoot expressionRoot, Expression<Func<TEntity, object>> columnExpression) where TEntity : BaseEntity
    {
        var type = typeof(TEntity);

        var columnExpressionBody = columnExpression.Body;

        if (columnExpression.Body is UnaryExpression unary && columnExpression.Body.NodeType == ExpressionType.Convert)
            columnExpressionBody = unary.Operand;

        if (columnExpressionBody is not MemberExpression member)
            throw new ArgumentException($"Expression '{columnExpression}' refers to a method, not a property.");

        if (member.Member is not PropertyInfo propInfo)
            throw new ArgumentException($"Expression '{columnExpression}' refers to a field, not a property.");

        var tableSyntax = expressionRoot.Table(NameCompatibilityManager.GetTableName(type));
        var propertyName = propInfo.Name;
        return tableSyntax.Column(NameCompatibilityManager.GetColumnName(type, propertyName));
    }
